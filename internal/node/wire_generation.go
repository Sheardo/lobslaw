...
func (n *Node) wireSpeakTools(builtins *compute.Builtins) error {
	...
	for _, ep := range eps {
		...
		cfgs = append(cfgs, compute.SpeakConfig{Label: ep.label, Driver: d, Resolver: resolver})
	}
	if len(cfgs) == 0 {
		return nil
	}
...
func (n *Node) wireImageTools(builtins *compute.Builtins) error {
	...
	for _, ep := range eps {
		...
		cfgs = append(cfgs, compute.ImageConfig{Label: ep.label, Driver: d, Resolver: resolver})
	}
	if len(cfgs) == 0 {
		return nil
	}
