...
func whisperAudioFor(t *testing.T, endpoint string) AudioDriver {
	t.Helper()
	d, err := WhisperAudioFactory(AudioDriverConfig{
		Endpoint:   endpoint,
		Model:      "whisper-1",
...
func chatMultimodalAudioFor(t *testing.T, endpoint string) AudioDriver {
	t.Helper()
	d, err := ChatMultimodalAudioFactory(AudioDriverConfig{
		Endpoint:   endpoint,
...
func TestAudioFactoriesNeedAnEndpointAndModel(t *testing.T) {
	t.Parallel()
	for name, f := range map[string]AudioDriverFactory{
		"whisper":        WhisperAudioFactory,
		"chatmultimodal": ChatMultimodalAudioFactory,
	} {
		if _, err := f(AudioDriverConfig{Model: "m"}); err == nil {
			t.Errorf("%s: no endpoint was accepted", name)
		}
		if _, err := f(AudioDriverConfig{Endpoint: "http://x"}); err == nil {
...
func TestTheMockAudioDriverNeedsNoNetwork(t *testing.T) {
	t.Parallel()
	d, err := MockAudioFactory(AudioDriverConfig{})
	if err != nil {
		t.Fatal(err)
	}
	got, err := d.Transcribe(t.Context(), AudioRequest{Filename: "voice.ogg"})
	if err != nil {
		t.Fatal(err)
	}
	if got == "" {
...
func TestBothAudioDriversPresentTheirCredential(t *testing.T) {
	t.Parallel()
	for name, build := range map[string]func(string) (AudioDriver, error){
		"whisper": func(url string) (AudioDriver, error) {
			return WhisperAudioFactory(AudioDriverConfig{
				Endpoint: url, Model: "whisper-1",
				Credential: NewBearerCredential("sk-test"),
			})
		},
		"chatmultimodal": func(url string) (AudioDriver, error) {
			return ChatMultimodalAudioFactory(AudioDriverConfig{
		...
		if _, err := d.Transcribe(t.Context(), AudioRequest{
