...
func (d *countingImageDriver) Generate(context.Context, ImageRequest) (*Artifact, error) {
	d.calls++
	return &Artifact{Kind: ArtifactInline, Bytes: mockPNG(), MIME: "image/png"}, nil
}
...
func registerImageTool(t *testing.T, env *testEnv, d ImageDriver) {
	t.Helper()
	b := NewBuiltins()
...
func TestGatingImagesLeavesOtherModalitiesAlone(t *testing.T) {
	...
	b := NewBuiltins()
	sd, _ := MockSpeakFactory(SpeakDriverConfig{})
