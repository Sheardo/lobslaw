...
func TestAGenerationEmitsASpanCarryingItsUnit(t *testing.T) {
	...
	CollectGeneration(ctx, "video-vendor", "veo-1",
		MeteredUsage(UnitVideoSeconds, 8, BillingBalance),
		types.ProviderPricing{UnitUSD: map[string]float64{string(UnitVideoSeconds): 0.25}},
		time.Now(), nil)
	...
	if got.Unit != string(UnitVideoSeconds) || got.Quantity != 8 {
...
func TestAPlanBilledCallIsMarkedAndCounted(t *testing.T) {
	...
	CollectGeneration(ctx, "plan-vendor", "veo-1",
		MeteredUsage(UnitVideoSeconds, 60, BillingPlan),
		types.ProviderPricing{UnitUSD: map[string]float64{string(UnitVideoSeconds): 1.00}},
		time.Now(), nil)

	got := waitForSpans(t, sink, 1)[0]
	if got.BilledTo != string(BillingPlan) {
...
func TestAFailedGenerationSpansButDoesNotCharge(t *testing.T) {
	...
	CollectGeneration(ctx, "vendor", "m",
		MeteredUsage(UnitImages, 1, BillingBalance),
		types.ProviderPricing{UnitUSD: map[string]float64{string(UnitImages): 5}},
...
func TestOneCallFeedsBothTheBudgetAndTheTrace(t *testing.T) {
	...
	CollectGeneration(ctx, "vendor", "m",
		MeteredUsage(UnitImages, 2, BillingBalance),
...
func TestAGenerationWorksWithNoRecorder(t *testing.T) {
	...
	CollectGeneration(ctx, "vendor", "m",
		MeteredUsage(UnitImages, 1, BillingBalance), types.ProviderPricing{},
		time.Now(), nil)
...
// Asserting on CollectGeneration alone would pass with the builtins
...
func TestGeneratingAnImageEmitsASpan(t *testing.T) {
	...
	got := waitForSpans(t, sink, 1)[0]
	if got.Kind != trace.KindGeneration || got.Unit != string(UnitImages) {
