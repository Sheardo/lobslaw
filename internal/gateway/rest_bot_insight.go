...
func (s *Server) handleBotRoutines(w http.ResponseWriter, r *http.Request, botID string) {
	...
	writeJSONBody(w, map[string]any{"routines": out})
}
...
func (s *Server) handleBotMemory(w http.ResponseWriter, r *http.Request, botID string) {
	...
	writeJSONBody(w, map[string]any{"records": records, "total": total})
}

// writeJSONBody is the one-line success path these handlers share.
